...
define("USE_SHOP", false); //영카트 사용여부

define("USE_MOBILE", false); //모바일 사용여부

define("USE_MOVE_MOBILE", false); //모바일 자동이동 (index.html 페이지에서 자동이동)

define("USE_MOVE_MOBILE_FROM_HEAD", false); //모바일 자동이동 (모든페이지에서 자동이동 PC페이지 확인은 ?pcv=1)

define("USE_NAVERPAY", false); //네이버페이 설정 표시
...
if(is_file($g4["path"]."/app_alimi/index.php")){
	$g4['app_path'] = "/app_alimi";
} else {
	$g4['app_path'] = "/app_board";
}

$g4['app_analytics_path'] = "/app_analytics";

//회원가입 & 관리자 회원정보에서 회원성별 사용시 true로 변경
$g4["use_mb_sex"] = false;

// 비밀글 사용안하는 게시판일 경우 비회원 패스워드 input입력창을 표시하고자하는 게시판의 bo_table을 넣습니다 
// 아래에 입력되지 않은 게시판일 경우(비밀글 사용안함 게시판) 비회원이 글 작성 시 패스워드 입력창이 표시되지 않으며 view화면에서 글쓰기/삭제 버튼이 안보입니다.
$bo_use_password_input = array(
	"0_0_0_0"
	, "0_0_0_1"
);
?>
